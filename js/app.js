...
class DB extends PouchDB {
	...
	sync() {
	PouchDB.sync(this.name, this.remote, {live: true, retry: true});
	}
}

class Message {
	constructor({date, author, text, likedBy, _rev, _id}) {
		...
		Object.defineProperty(this, '_likedBy', {value: likedBySet});
  ...
  /**
	...
	**/
	update(db, {likedBy, _rev, _conflicts = []}) {
  ...
  /**
	...
	**/
	resolveConflicts(db, conflicts) {
		const docPromises = conflicts.map(_rev => db.get(this._id, {rev: _rev}));
